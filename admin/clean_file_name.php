...
function remove_specialchar($str)
{
...
{
    $file_name = preg_replace("/\.[^.\s]{3,4}$/", "", $name);//enlever l'extension
    $file_ext = substr($name, strrpos($name, '.'));
    $file_ext = str_replace('.','',$file_ext);
    $file_name = remove_specialchar($file_name);
    $random_num = time();
    $new_file_name = $file_name.'-'.$random_num.'.'.$file_ext;
